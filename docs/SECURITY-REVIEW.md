...
Remaining phase 2.1 work: exercise a native image protocol request using the
current source build, review URL and file-opening behavior on every announced
OS, and rerun the adversarial corpus and dependency audit on the final release
commit and binaries. These are release gates, not inferred from this review.
